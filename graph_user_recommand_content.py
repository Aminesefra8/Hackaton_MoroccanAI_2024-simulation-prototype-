...
class Info_Type(BaseModel):
    ...
    # Boolean indicating if motivations or methods are detected in the text
    is_motivation: str = Field(
        description=(
            "A boolean value indicating whether the text includes mentions of motivations or methods for improving mental health. "
            "Set to 'True' if any motivations or strategies are found in the text; otherwise, 'False'."
        ),
        enum=["True", "False"],
    )

    # Extracted motivations and methods
    motivations_info: str = Field(
        description=(
            "Detailed information extracted from the text related to motivations for mental health, "
            "methods to fight bad mental health, tips, strategies, or actionable advice. "
            "This includes a description or list of motivations and any suggestions explicitly or implicitly provided."
        )
    )
...
def output_final(state: State):
    ...
    output_dict = {
        ...
        "is_motivation": state["extract_info_output"].is_motivation,
        "motivations_info": state["extract_info_output"].motivations_info
